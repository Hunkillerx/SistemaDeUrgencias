...
class Hospital : Comparable<Hospital> {
    ...
    fun darUbicacion() = ubicacion
    fun darAccidentado() = accidentados
...
object SistemaDeEmergencias {
    ...
    fun agregarAmbulancia(codigo: Int, calle: Int, carrera: Int) {
        if (!existeAbulancia(codigo)) {
            val ambulancia = Ambulancia(codigo, Punto(calle, carrera))
    ...
    fun obtenerPacientes(id: Int): Array<String> {
        val accidentados_hospital = hospitales.get(id).darAccidentado()
        val resultado = Array<String>(accidentados_hospital.size) { "" }
        for (i in 0 until accidentados_hospital.size) {
            resultado[i] = accidentados_hospital[i].darNombre()
        }
        return resultado
    }

    fun obtenerAmbulanciasLibres(): Array<Int> {
        val resultado = Array<Int>(ambulancias.size){0}
        for (i in 0 until ambulancias.size) {
            if (ambulancias[i].darEstado() == false) {
                resultado[i] = ambulancias[i].darCodigo()
            }
        }
        return resultado
    ...
    fun obtenerAmbulanciasOcupadas(): Array<Int> {
        val resultado = Array<Int>(ambulancias.size){0}
        for (i in 0 until ambulancias.size) {
            if (ambulancias[i].darEstado() == true) {
                resultado[i] = ambulancias[i].darCodigo()
            }
        }
        return resultado
    ...
    fun obtenerDatosHospital(id:Int):Hospital? {
        var resultado: Hospital? = null
        for (i in 0 until hospitales.size) {
            if (hospitales[i].darCodigo() == id) {
                resultado = hospitales[i]
            }
        }
        return resultado
    }
